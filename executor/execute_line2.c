...
/*   Updated: 2023/09/19 15:09:30 by rdolzi           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	execute_recursivev2(t_node *node)
{
	...
	while (1)
	{
				...
				&& node->done_lock == 0))
		{
			...
			if (ft_get_op(node) == PIPE || (node == go_to_starter_node(
						node->shell->tree->left)) || (norm_check(node)))
				ft_single_cmd2(node, ft_do_redir2);
			if (node->shell->lvl_subshell > 0)
				ft_clean_exit(node->shell, NULL, node->shell->exit_status, 1);
			return ;
		}
		if (!node || node->done_lock == 1 || (node && ft_back_node(
				...
				&& node->shell->lvl_subshell != 0))
		{
			if (!node)
			{
				...
				ft_clean_exit(node->shell, NULL, node->shell->exit_status, 1);
			if ((node && node->done_lock == 1 && ft_back_node(
						node)->lvl_subshell >= node->shell->lvl_subshell
					&& node->shell->lvl_subshell != 0))
				ft_clean_exit(node->shell, NULL, node->shell->exit_status, 1);
			else
