...
/*   Updated: 2023/08/26 19:24:08 by rdolzi           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
    ...
    while (++idx < ft_strlen(shell->str))
    {
        if (shell->str[idx] == '(' && !in_quotes_str(shell, idx))
        {
            ...
            if (recursive(shell, par, left_p, idx))
                return (1);
            if ((op_before(shell, idx) < char_before(shell, idx)) || (op_before(shell, idx) && !char_before(shell, op_before(shell, idx))))
                return (1);
            if (!op_before(shell, idx) && char_before(shell, idx))
                return (1);
            if (op_after(shell, idx) && !char_after(shell, op_after(shell, idx), 0, "><"))
                return (1);
