...
// controlla il next op a partire da un idx, ma va sempre verso sinistra
int get_idx_next_op(t_node *node, int idx)
{
    char *chars;

    chars = "|&";
    while (idx-- > 0) // se fault per il primo? non dovrebbe esserci(check error iniziale)
    {
...
// evita il seg. fault
int is_last_char(t_node *node, int idx) // da modificare, deve guardare generic str
{
...
int there_is_logic_operator(t_node *node)
{
    ...
    while (--idx > 0) // no check ultimo char perche check errore in fase iniziale
    {
        next_op = get_idx_next_op(node, idx); // ritorna il primo char se || o &&
